...
- `/speak agent` mode: the agent itself decides what to say by emitting `pk-speak "..."` inline at the end of its turn; no second LLM summarizer pass, no auto-speak watcher; agent silence = no audio; classic `/speak on` is unchanged and remains the path for remote/phone turns
- `pk-speak` CLI: synthesizes text and plays audio from any shell; supports `--voice <name>`, `--no-play`, `--no-wait`, `--output <path>`, `--rewrite`, `--help`, `--version`; exit 0 on success, exit 1 with a clean single-line stderr message on failure, exit 2 when no text is provided
- `audio-playback.ts` shared module exporting `getPlayerInvocation` (pure, platform-aware: PowerShell MediaPlayer on win32, afplay on darwin, ffplay on linux) and `playAudio` (cross-platform spawn with optional wait); used by both the extension and the `pk-speak` CLI
- `speech-preamble.ts` exporting `PK_SPEAK_PREAMBLE` — a self-contained instruction string that pi injects automatically in `/speak agent` mode and that codex/claude-code operators paste into their own config (`AGENTS.md` or `CLAUDE.md`) for the same agent-driven speech behavior without per-runtime plugins
- `docs/AGENT_SPEAK.md` covering the rationale for agent-driven speech, the full preamble text, and concrete wiring instructions for each runtime (pi, codex, oh-my-pi, claude code)
- `pk-speak-mcp` stdio MCP server: a thin adapter that shells out to the `pk-speak` CLI and exposes a single `speak` tool with `{ text, voice? }` input; intended for clients that cannot invoke a shell directly (Claude Code without Bash, restricted codex environments); all diagnostics go to stderr, nothing reaches stdout except JSON-RPC; available as the `pk-speak-mcp` bin after `npm i -g pi-speak-pk`
- `integrations/` directory bundling ready-to-use integration configs: `claude-code/` (`CLAUDE.md` paste snippet, optional `.mcp.json` and SessionStart hook), `codex/` (`AGENTS.md` paste snippet and `~/.codex/config.toml` `[mcp_servers.pk-speak]` stanza), and `oh-my-pi/` (`AGENTS.md` paste snippet plus the same `config.toml` stanza, since oh-my-pi shares the codex config path)
