// @ts-check
/**
 * AudioWorkletProcessor: AudioContext rate -> 16 kHz Int16 PCM chunks.
 *
 * HF realtime methodology (smolagents/hf-realtime-voice mic-capture):
 *   - ~40 ms chunks (640 samples @ 16 kHz)
 *   - 48->16 fast path with 3-tap boxcar lowpass; linear-interp fallback
 *   - optional noise gate (dBFS threshold, fast attack / hold / slow release)
 *   - mute via {kind:"enable"} without tearing down the graph
 *   - posts {kind:"level", rms} every chunk for UI meters
 *
 * Wire format for the main thread: transferable ArrayBuffer of Int16 LE PCM
 * (already at 16 kHz). The main thread only stamps the 4-byte sequence header.
 */
