...
| `herdr-client.ts` | Async Herdr CLI client and wire parser: nested socket-response extraction plus semantic agent list/read/prompt/focus helpers; finds a sibling pk-herd debug/release binary only as a development fallback before `herdr` on `PATH` |
| `session-workspace.ts` | Live Herdr session facade for `/v1/sessions/live*`: opaque native-session-first IDs, bounded output tails, SSE state/tail updates, serialized prompt/focus/resume mutations guarded by expected Herdr revisions and durable two-phase idempotency receipts |
...
- **Live Herdr session workspace**: `/hub/` is the phone/browser surface for Herdr-owned live agent sessions. It uses `GET /v1/sessions/live` for the authoritative list, `GET /v1/sessions/live/:id` for a bounded output tail, `GET /v1/sessions/live/:id/stream` for SSE state/tail updates, and `POST /v1/sessions/live/:id/prompt|focus|resume` for semantic actions. Mutations require `X-Pi-Speak-Idempotency-Key` plus the current `expectedRevision`; pending and successful receipts persist in `session-workspace-mutations.json` so a retry after a gateway restart either replays the recorded result or stops with `mutation_outcome_unknown` instead of double-submitting. Session IDs hash the native Herdr session reference when one exists and fall back to pane coordinates only for anonymous agents; the DTO and UI do not expose workstation pane targets or native session values.
...
- **Remote audio**: Browser mic requires HTTPS origin (use Tailscale Serve or a tunnel). Direct daemon-verified tailnet peers are tokenless. Proxied Serve identity trust requires explicit `PI_SPEAK_TRUST_TAILSCALE_SERVE_PROXY=1`; other loopback proxy traffic and Funnel remain token-gated.
