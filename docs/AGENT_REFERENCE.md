...
| `realtime-gateway.ts` | Live conversational assistant on `/v1/live`: `REALTIME_SYSTEM_PROMPT`, `buildRealtimeTools` / `dispatchRealtimeToolCall` (session/hub/workspace reads + `web_search` + `camera_snapshot` + approval-gated mutations). Default upstream HF speech-to-speech (OpenAI-Realtime wire) when an S2S URL or backend is configured; Gemini Live fallback otherwise, via `live-backend.ts`. |
...
| `openai-realtime-live.ts` | OpenAI-Realtime / HF S2S adapter; connect URL from `PI_SPEAK_OPENAI_REALTIME_URL` / `SPEECH_TO_SPEECH_URL`, defaulting to `ws://localhost:8765/v1/realtime`. |
