...
- **Purpose:** Authenticated HTTP API and mobile web-app host.
- **Key exports/symbols:** Gateway server creation and handlers for `/v1/*`, static hosting for `/app/`, status/diagnostics, turn submission, routing, sessions, workspace, events, and Agent Hub operations.
- **Driven by:** Browser/PWA, Android, Telegram bridge, and other HTTP clients. It validates/authenticates requests, routes turns into the extension runtime, and exposes the gateway API.
...
| `/v1/live` | Establish the Live voice/audio interaction path. |
...
Requests are authenticated with `PI_SPEAK_HTTP_TOKEN` where required. The same gateway surface serves the browser, Android, Telegram bridge, and standalone daemon clients.

## 6. Android and PWA clients

The web app is hosted at `/app/`; it provides the broad browser remote for text turns, voice uploads, target selection, setup, reply audio, session management, events, and workspace browsing. The native APK is downloadable at `/download/pi-speak.apk`; it stores machine profiles and connection/auth settings locally, offers native text/voice controls, and mirrors session routing, events, workspace, and Agent Hub operations. The Boox variant is an e-ink-oriented native surface with adapted layouts and controls for the Boox device. Unified Remote is a lightweight remote-control surface for quick machine/control actions rather than the full session and Agent Hub management experience. All clients call the same gateway; the differences are local device capabilities and UI, not separate agent backends.
...
| `PI_SPEAK_GEMINI_BACKEND` | Choose Gemini API backend. | `developer-api`/`developer`/`api` or `vertex`/`vertexai`/`gcloud`. | `gemini-live-turn.ts` |
