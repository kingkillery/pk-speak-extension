...
File naming: `<plan-root>/NNN-short-slug.md`, numbered in recommended execution order. `<plan-root>` is `plans/`, or `advisor-plans/` when `plans/` already exists for an unrelated purpose (see `SKILL.md`).
...
> in `<plan-root>/README.md` — unless a reviewer dispatched you and told you they
> maintain the index.
>
...
- **Effort**: S | M | L
- **Risk**: LOW | MED | HIGH
- **Depends on**: `<plan-root>`/NNN-*.md (or "none")
...
- [ ] `<plan-root>/README.md` status row updated
...
## Index file: `<plan-root>/README.md`
