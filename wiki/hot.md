...
- HF speech-to-speech is the default S2S upstream: any S2S URL (or `PI_SPEAK_LIVE_BACKEND=hf|openai-realtime`) selects it, URL defaults to `ws://localhost:8765/v1/realtime`; Gemini is the no-config fallback. Clients always use `/v1/live`.
